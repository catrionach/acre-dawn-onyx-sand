...
import type { FloorState, Note, QtCause, UnitStatus, WoStatus } from "./types";
import { QT_CAUSES } from "./types";
import {
  isBuildReportGrid,
  isLookupWorkbook,
  parseBuildReportGrid,
  parseLookupSheets,
  type ParsedLookup,
  type BuildReportRow,
} from "./build-lookup";
import { applyBuildReport, replaceLookup } from "./build-store";
...
function cell(rec: Record<string, string>, ...keys: string[]): string {
  for (const k of keys) {
    const v = rec[k];
...
function asQty(value: string, fallback = 1): number {
  if (!value.trim()) return fallback;
  const n = Number.parseInt(value, 10);
  return Number.isFinite(n) && n >= 1 ? n : fallback;
}

function asHours(value: string, fallback = 0): number {
  if (!value.trim()) return fallback;
  const n = Number.parseFloat(value);
  return Number.isFinite(n) ? n : fallback;
}

function asWeeks(value: string): number | null {
  if (!value.trim()) return null;
  const n = Number.parseFloat(value);
  return Number.isFinite(n) ? n : null;
}

function woStatusOf(value: string, fallback: WoStatus = "pending"): WoStatus {
...
function unitStatusOf(value: string): UnitStatus {
  const v = value.trim().toLowerCase().replace(/[_-]+/g, " ");
  if (v === "in build" || v === "on shelf" || v === "shipped") return v;
  return "in build";
}

function orderDateOf(rec: Record<string, string>): string | null {
  return parseFlexibleDate(
    cell(rec, "order_date", "ordered", "date_ordered", "ordered_date", "date"),
  );
}

function soStatusOf(
  value: string,
): "open" | "waiting_on_customer" | "despatched" | "cancelled" {
  const v = value.trim().toLowerCase().replace(/\s+/g, "_");
  if (v === "waiting_on_customer" || v === "waiting" || v === "on_hold") {
...
function ticketStatusOf(value: string): "open" | "closed" {
  return value.trim().toLowerCase() === "closed" ? "closed" : "open";
}

function parseCauses(value: string): QtCause[] {
...
function isOpen(status: WoStatus): boolean {
...
type Bucket = Record<SheetKey, Record<string, string>[]>;
...
function collect(files: ImportFile[]): {
  bucket: Bucket;
  ...
  for (const file of files) {
    try {
      if (file.kind === "xlsx") {
        ...
        if (isLookupWorkbook(file.name, [{ name: file.name, rows: grid }])) {
          lookups.push(parseLookupSheets([{ name: file.name, rows: grid }]));
          continue;
        }
  ...
  return { bucket, errors, lookups, reports };
}

async function nextQt(sql: Sql): Promise<string> {
  const rows = await sql<{ ticket_number: string }>`select ticket_number from quality_tickets`;
  let max = 0;
...
export async function applyFloorImport(
  files: ImportFile[],
  loadState: () => Promise<FloorState>,
): Promise<{ report: ImportReport; state: FloorState }> {
  ...
  if (reports.length) {
    const result = await applyBuildReport(sql, reports, "Load", (row, message) => {
      report.errors.push({ sheet: "build_report", row, message });
    });
  ...
  for (const rec of bucket.parts) {
    ...
    if (!partNumber) {
      report.errors.push({ sheet: "parts", row: "", message: "Missing part_number" });
      continue;
    }
    try {
  ...
  for (const rec of bucket.work_orders) {
    ...
    if (!woNumber) {
      report.errors.push({ sheet: "work_orders", row: "", message: "Missing wo_number" });
      continue;
    }
    try {
      const existing = await sql<Record<string, unknown>>`
      ...
      `;
      const status = woStatusOf(
        cell(rec, "status"),
        existing[0] ? woStatusOf(String(existing[0].status ?? "pending")) : "pending",
      );
      const today = todayIso();
      if (existing[0]) {
        const cur = existing[0];
        let dateStarted = (cur.date_started as string | null) ?? null;
        let dateClosed = (cur.date_closed as string | null) ?? null;
        ...
        if (status === "closed") {
          dateClosed = parseFlexibleDate(cell(rec, "date_closed")) ?? dateClosed ?? today;
        } else if (prev === "closed") {
          dateClosed = null;
        }
        await sql`
          update work_orders set
            ...
            status = ${status},
            build_time_hours = ${
              cell(rec, "build_time_hours")
                ? asHours(cell(rec, "build_time_hours"))
                : ((cur.build_time_hours as number | null) ?? null)
            },
            ...
            notes_to_production = ${
              cell(rec, "notes_to_production") || String(cur.notes_to_production ?? "")
            },
            notes_from_sales = ${cell(rec, "notes_from_sales") || String(cur.notes_from_sales ?? "")},
            customer_need_date = ${
              parseFlexibleDate(cell(rec, "customer_need_date")) ??
              (cur.customer_need_date as string | null) ??
              null
            }
          where wo_number = ${woNumber}
        `;
        if (isOpen(prev) && !isOpen(status)) {
          await sql`delete from build_order where wo_number = ${woNumber}`;
        }
        if (!isOpen(prev) && isOpen(status)) {
          const have = await sql<{ wo_number: string }>`
            select wo_number from build_order where wo_number = ${woNumber}
          `;
          if (!have.length) {
            const maxRows = await sql<{ max_pos: number | null }>`
              select max(position) as max_pos from build_order
            `;
            await sql`insert into build_order (wo_number, position) values (${woNumber}, ${(maxRows[0]?.max_pos ?? -1) + 1})`;
          }
        }
        ...
        const dateStarted =
          parseFlexibleDate(cell(rec, "date_started")) ?? (status === "active" ? dateAdded : null);
        const dateClosed =
          parseFlexibleDate(cell(rec, "date_closed")) ?? (status === "closed" ? today : null);
        await sql`
  ...
  for (const rec of bucket.units) {
    ...
    if (!woNumber) {
      report.errors.push({ sheet: "units", row: "", message: "Missing work_order_number" });
      continue;
    }
    try {
      const wo = await sql<{ wo_number: string }>`
      ...
      if (unitId) {
        ...
        if (existing[0]) {
          ...
          await sql`
            update units set
              ...
              sales_order_number = ${
                cell(rec, "sales_order_number", "so_number") ||
                (cur.sales_order_number as string | null)
              },
              despatch_date = ${
                parseFlexibleDate(cell(rec, "despatch_date")) ?? (cur.despatch_date as string | null)
              }
        ...
        const taken = await sql<{ unit_id: string }>`
  ...
  for (const rec of bucket.quality_tickets) {
    ...
    try {
      let woPart = "";
      if (woNumber) {
        const wo = await sql<{ part: string }>`
      ...
      if (ticketNumber) {
        const existing = await sql<{ ticket_number: string }>`
  ...
  for (const rec of bucket.sales_orders) {
    ...
    if (!soNumber) {
      report.errors.push({ sheet: "sales_orders", row: "", message: "Missing so_number" });
      continue;
    }
    try {
      const existing = await sql<Record<string, unknown>>`
      ...
      if (existing[0]) {
        ...
        await sql`
          update sales_orders set
            ...
            order_date = ${orderDate ?? (cur.order_date as string | null)},
            lead_time_weeks = ${lead ?? (cur.lead_time_weeks as number | null)},
            target_despatch = ${target ?? (cur.target_despatch as string | null)},
            target_despatch_is_override = ${
              cell(rec, "target_despatch") ? isOverride : Boolean(cur.target_despatch_is_override)
            },
  ...
  const lineRows = [
    ...bucket.sales_lines,
    ...bucket.sales_orders.filter((rec) => cell(rec, "part", "part_number", "item")),
  ];
  for (const rec of lineRows) {
    let soNumber = asId(
      cell(rec, "so_number", "sage_number", "sage_id", "built_in_sage"),
    );
    ...
    try {
      const so = await sql<{ so_number: string }>`
  ...
  for (const rec of bucket.hardware_history) {
    ...
    try {
      const rows = await sql<{ hardware_history: unknown }>`
      ...
      const stamp = parseFlexibleStamp(cell(rec, "date")) ?? `${todayIso()}T09:00`;
      const note: Note = { date: stamp, author: cell(rec, "author"), text };
      let current: Note[] = [];
      const raw = rows[0].hardware_history;
      if (Array.isArray(raw)) current = raw as Note[];
      else if (typeof raw === "string") {
        try {
          current = JSON.parse(raw) as Note[];
        } catch {
          current = [];
        }
      }
      const dupe = current.some(
        (n) => n.date === note.date && n.author === note.author && n.text === note.text,
      );
      if (dupe) {
  ...
  if (bucket.build_order.length) {
    try {
      const wanted = bucket.build_order
        .map((rec) => ({
      ...
      const open = await sql<{ wo_number: string; status: string }>`
      ...
      for (let i = 0; i < next.length; i += 1) {
        await sql`insert into build_order (wo_number, position) values (${next[i]}, ${i})`;
      }
  ...
  return { report, state: await loadState() };
}
